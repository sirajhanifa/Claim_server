...
const getCreditedClaims = async (req, res) => {
  try {
    const result = await Claim.aggregate([
      {
        $match: { status: "Credited" }  // Only credited claims
      },
      {
        $group: {
          _id: null,
...
const getPendingClaims = async (req, res) => {
  try {
    const result = await Claim.aggregate([
      {
        $match: { status: "Pending" } // Only pending claims
      },
      {
        $group: {
          _id: null,
...
const getAwaitingClaims = async (req, res) => {
  try {
    ...
    const result = await Claim.aggregate([
      {
        $match: {
          status: { $ne: "Credited" },      // NOT credited (pending or processing)
          entry_date: { $lte: sevenDaysAgo } // older than 7 days
        }
      },
      {
        $group: {
